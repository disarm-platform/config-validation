...
export function validate_edges(
  config: TConfig,
  path_map: TPathMap[],
  edge_definitions: TEdgeDefinition[]
): TStandardEdgeResponse[] {
  // create helpers
...
function validate_edge(
  config: TConfig,
  nodes: MappedNode[],
  edge_definition: TEdgeDefinition
): TStandardEdgeResponse {
  let nodes_exist: TNodeResponse;

  // Tell me about this Edge
  let edge_required = edge_definition.required;
  const edge_name = `${edge_definition.source_node_name}_${
    edge_definition.target_node_name
  }`;

  // Basic checks for Node existence
  const source_node = nodes.find(
    n => n.name === edge_definition.source_node_name
  );
  const target_node = nodes.find(
    n => n.name === edge_definition.target_node_name
  );
  ...
  if (!source_node_exists) {
    ...
    return determine_edge_result(
      edge_definition,
      nodes_exist,
      edge_required,
      []
    ); // edge cannot be required if source node is not there
  }
...
export function determine_edge_result(
  edge_definition: TEdgeDefinition,
  node_response: TNodeResponse,
  edge_required: boolean,
  custom_edge_responses: TCustomEdgeResponses
): TStandardEdgeResponse {
  ...
  const edge_passes =
    custom_edge_responses &&
    custom_edge_responses.every(r => r.status === ECustomEdgeStatus.Green);
