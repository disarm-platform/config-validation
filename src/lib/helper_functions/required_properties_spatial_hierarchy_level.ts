...
export function required_properties_on_sh_level(spatial_hierarchy_level: TLevel, geodata_properties_summary: TFieldSummary[]): TCustomEdgeResponse {
  const required_fields = ['group_by_field', 'field_name']
    .map(n => get(spatial_hierarchy_level, n))
    .filter(i => i);

  const all_required_fields_exist = required_fields.map((field_name): TCustomEdgeResponse => {
  ...
  if (all_required_fields_exist.every(e => e.status === ECustomEdgeStatus.Green)) {
    ...
    const messages_string = all_required_fields_exist.map(e => e.message).join(', ')
    return {
