...
export function markers_valid(
  ...
  const planning_level_name_valid = geodata_layer_names.includes(
    markers.planning_level_name
  );
  if (!planning_level_name_valid) {
    return {
      message: `planning_level_name invalid: Cannot find ${
        markers.planning_level_name
      } in markers ${JSON.stringify(markers)}`,
      status: ECustomEdgeStatus.Red
    };
  }
  // record_location_selection_level_name is a level
  const record_location_selection_level_name_valid = geodata_layer_names.includes(
    markers.record_location_selection_level_name
  );
  if (!record_location_selection_level_name_valid) {
    return {
      message: `record_location_selection_level_name invalid: Cannot find ${
        markers.planning_level_name
      } in markers ${JSON.stringify(markers)}`,
      status: ECustomEdgeStatus.Red
    };
  ...
  const denominator_fields_valid = denominator_fields.every(f =>
    level_fields.includes(f)
  );
