...
export function valid_id_fields(
  spatial_hierarchy: TSpatialHierarchy
): TCustomEdgeResponse[] {
  const geodata_summary: TGeodataSummary = spatial_hierarchy.geodata_summary;
  return spatial_hierarchy.levels.map(level => {
    ...
    const support_messages = [
      ...
      `Summary: ${JSON.stringify(level_summary)}`
    ];

    const valid_level_field_name = unique_omnipresent_reliable(
      find(level.field_name, level_summary)
    );
    ...
    const valid_level_display_field_name = unique_omnipresent_reliable(
      find(level.display_field_name, level_summary)
    );
    ...
    const valid_level_group_by_field = level.group_by_field
      ? omnipresent_reliable(find(level.group_by_field, level_summary))
      : true;
