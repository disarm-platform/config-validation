...
test('valid config number', t => {
  ...
  const result: TNodeResponse = runNumberValidation({number_config: validConfig})
...
test('invalid config number', t => {
  ...
  const result: TNodeResponse = runNumberValidation({number_config: invalidConfig})
...
test('valid config string', t => {
  ...
  const result: TNodeResponse = runOptionalStringValidation({string_config: validConfig})
...
test('internally invalid config string', async t => {
  ...
  // @ts-ignore
  const result: TNodeResponse = runOptionalStringValidation({ string_config: internallyInvalidConfig })
