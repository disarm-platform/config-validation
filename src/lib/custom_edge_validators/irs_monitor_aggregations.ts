...
export function irs_monitor_aggregations(
  config: TConfig
): TCustomEdgeResponses {
  ...
  const required_aggregations = extract_aggregations_from_irs_monitor(
    irs_monitor_config
  );
...
function extract_aggregations_from_irs_monitor(
  irs_monitor_config: TIrsMonitor
): string[] {
  ...
  for (const chart of irs_monitor_config.charts) {
    ...
    if (chart.options.multi_series) {
      chart.options.multi_series.forEach(({ aggregation_name }) =>
        chart_aggregations.push(aggregation_name)
      );
    }
  }
