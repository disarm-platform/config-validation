...
export function irs_monitor_fields_helper(config: TConfig): TCustomEdgeResponses {
  ...
  const response_point_fields = irs_monitor_config.map.response_point_fields.filter(responde_point_field => {
    // only get the ones that reference data we can check against.
    return responde_point_field.startsWith('form_data.') || responde_point_field.startsWith('_decorated.')
  })
  ...
  const fields_were_interested_in = [...response_point_fields, ...generate_series_from_fields].map(field => {
