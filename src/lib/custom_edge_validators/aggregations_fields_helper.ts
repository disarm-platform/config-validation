...
export function aggregations_fields_helper(
  config: TConfig
): TCustomEdgeResponses {
  ...
  const required_fields = flatten(
    aggregations_config.map(aggregation => {
      const aggregationVariables = expression_variables(
        aggregation.numerator_expr
      );
